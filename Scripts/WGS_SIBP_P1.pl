...
if ($type =~ /QualityCheck/i){
	system "rm -rf $Res/$OUT\t$Res/$MQC\n";
	if (!-d "$Res/$OUT"||"$Res/$MQC"){
	system "mkdir $Res/$OUT\t$Res/$MQC\n";}
open(INPUT,$In);
...
system "multiqc $Res/$OUT/*_fastqc.zip -o $Res/$MQC\n";
	...
	system "rm -rf $Res/$Trim\t$Res/$Trim1\t$Res/$MQC1\n";
 	if (!-d "$Res/$Trim"||"$Res/$Trim1"||"$Res/$MQC1"){
	system "mkdir $Res/$Trim\t$Res/$Trim1\t$Res/$MQC1\n";}
open(INPUT,$In);
while ( my $rawData =<INPUT>){
chomp($rawData);
my ($ID,$pair1,$pair2) = split /\t/,$rawData;
...
system "java -jar /staff/res498/Soft/Trimmomatic-0.36/trimmomatic-0.36.jar PE -phred33 $PATH/$pair1 $PATH/$pair2 $Res/$Trim/$OUTpair1 $Res/$Trim/$OUTUnpair1 $Res/$Trim/$OUTpair2 $Res/$Trim/$OUTUnpair2 LEADING:$lead TRAILING:$trail SLIDINGWINDOW:$SlidWin MINLEN:$MinLen\n";
...
print "Finish Data Cleaning\n";
	...
	system "rm -rf $Res/$OUT\t$Res/$MQC\t$Res/$Trim\t$Res/$Trim1\t$Res/$MQC1\n";
	if (!-d "$Res/$OUT"||"$Res/$MQC"||"$Res/$Trim"||"$Res/$Trim1"||"$Res/$MQC1"){
	system "mkdir $Res/$OUT\t$Res/$MQC\t$Res/$Trim\t$Res/$Trim1\t$Res/$MQC1\n";}
